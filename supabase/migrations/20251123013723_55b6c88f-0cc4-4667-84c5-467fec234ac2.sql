-- Fix the premature trip completion bug
...
-- Recreate with correct logic: Only complete when BOTH parties have acted
...
BEGIN
  ...
  -- CRITICAL FIX: Only set status to 'completed' when BOTH parties have acted
  -- Either both completed OR both rated OR one completed and other rated
  IF ((NEW.rider_completed = true OR NEW.rider_rating IS NOT NULL) AND 
      (NEW.driver_completed = true OR NEW.driver_rating IS NOT NULL)) THEN
...
CREATE TRIGGER increment_completed_trips_trigger
