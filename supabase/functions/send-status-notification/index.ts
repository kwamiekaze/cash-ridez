...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userEmail, displayName, status }: StatusNotificationRequest = await req.json();
      ...
      : `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #fef2f2; border-left: 4px solid #ef4444; padding: 16px; margin: 24px 0;">
            <h3 style="margin-top: 0; color: #991b1b;">Common Reasons for Rejection:</h3>
            <ul style="color: #991b1b;">
              <li>ID image is unclear or unreadable</li>
              <li>Document type not accepted (we require government-issued ID)</li>
