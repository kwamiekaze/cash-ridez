-- Idempotent in-app "new trip near you" alert insertion.
--
-- The uniqueness rule is a PARTIAL unique index, so PostgREST's onConflict
-- cannot infer it (Postgres 42P10). This routine carries the matching
-- predicate, which makes a worker retry insert zero rows without error.
--
-- Applied live on 2026-09-10; kept here so the SQL is testable.
