  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        const {
          data: profileData
        } = await supabase.from("profiles").select("display_name, is_rider, is_driver").eq("id", user?.id).single();
      ...
      <div className="flex flex-col items-center justify-center p-4 min-h-[calc(100vh-4rem)]">
        ...
        <Card className="max-w-2xl w-full p-8">
          ...
          <div className="relative rounded-lg overflow-hidden bg-muted mb-6">
            <video autoPlay preload="metadata" muted playsInline className="w-full h-auto" src="/onboarding-intro.mov" controls>
        ...
        <div className="space-y-6">
          {isVerified ? <div>
              ...
              <Card className="p-6 border-primary bg-primary/5">
                <div className="flex flex-col items-center gap-4 text-center">
                  ...
                  <Button className="w-full bg-gradient-primary mt-4" size="lg" onClick={() => navigate("/dashboard")}>
                    Go to Dashboard
                  </Button>
                </div>
              </Card>
            </div> : profile?.verification_status === "pending" && profile?.id_image_url ? <div>
              ...
              <h2 className="text-xl font-semibold mb-4 text-center">Account Verification Required</h2>
          ...
          {!isVerified && <>
              <Button className="w-full bg-gradient-primary" size="lg" onClick={handleSubmit} disabled={uploading || !idFile || !selectedRole}>
                {uploading ? "Submitting..." : profile?.id_image_url ? "Resubmit for Verification" : "Submit for Verification"}
              </Button>
              {!selectedRole && <p className="text-sm text-destructive text-center mt-2">
